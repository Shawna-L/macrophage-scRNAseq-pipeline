# ================================================================
# SingleR-based macrophage annotation pipeline (mouse heart)
# ================================================================
# Author: Jingshuo Li
# Version: 1.0
# Description: End-to-end scRNA-seq workflow for mouse heart with
...
project_name <- "MouseHeart_SingleR"
data_dir     <- "data/mouse/heart"                     # raw 10X input
dag_path     <- "data/DAG_136_MouseGenes.csv"          # DAG list (first column = gene symbol)
ref_path     <- "refs/mouse/mca_heart_ref.rds"         # MCA heart SingleR reference (RDS)
results_dir  <- "results/mouse/heart/singler_pipeline" # output root
...
gsm_mouse_heart <- tryCatch({
  getGEO("GSM5687566", GSEMatrix = FALSE)
}, error = function(e) { message("GEO fetch skipped."); NULL })
if (!is.null(gsm_mouse_heart)) utils::str(gsm_mouse_heart)

# -------------------- Step 3: Load raw 10X gene expression data --------------------
mouse_heart <- Read10X(data.dir = data_dir) %>%
...
overlap_dag  <- intersect(rownames(mouse_heart), dag_genes)

# -------------------- Step 5: Add QC metrics --------------------
mouse_heart[["percent.mito"]] <- PercentageFeatureSet(mouse_heart, pattern = "^mt-")
mouse_heart[["percent.dag"]]  <- PercentageFeatureSet(mouse_heart, features = overlap_dag)

print(summary(mouse_heart$percent.mito))
print(summary(mouse_heart$percent.dag))
...
hist(mouse_heart$percent.mito, breaks = 50, col = "lightblue", main = "Mito %", xlab = "percent.mito"); dev.off()
png(file.path(qc_hist_dir, "Histogram_percent.dag.png"), width = 800, height = 600)
hist(mouse_heart$percent.dag, breaks = 50, col = "pink", main = "DAG %", xlab = "percent.dag"); dev.off()
...
plot1 <- FeatureScatter(mouse_heart, feature1 = "nCount_RNA", feature2 = "percent.mito")
plot2 <- FeatureScatter(mouse_heart, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot3 <- FeatureScatter(mouse_heart, feature1 = "nCount_RNA", feature2 = "percent.dag")
plot4 <- FeatureScatter(mouse_heart, feature1 = "percent.dag", feature2 = "percent.mito")
...
mouse_heart_filtered <- subset(mouse_heart,
                               subset = nFeature_RNA > 1000 &
                                 nFeature_RNA < 6000 &
                                 percent.mito   < 10 &
                                 percent.dag    < 5)
saveRDS(mouse_heart_filtered, file = file.path(results_dir, "mouse_heart_filtered.rds"))

# -------------------- Step 8: Normalization --------------------
mouse_heart_filtered <- NormalizeData(mouse_heart_filtered)

# -------------------- Step 9: Join layers (Seurat v5) --------------------
mouse_heart_filtered <- JoinLayers(mouse_heart_filtered)

# -------------------- Step 10: HVGs (mean.var.plot) --------------------
mouse_heart_meanvar <- FindVariableFeatures(
  mouse_heart_filtered,
  selection.method = "mean.var.plot",
  mean.cutoff = c(0.01, 4),
  dispersion.cutoff = c(0.3, Inf)
)
message("Variable features: ", length(VariableFeatures(mouse_heart_meanvar)))

output_dir_meanvar <- file.path(results_dir, "Method_meanvar")
dir.create(output_dir_meanvar, recursive = TRUE, showWarnings = FALSE)

# -------------------- Step 11: Scale + PCA --------------------
mouse_heart_meanvar <- ScaleData(mouse_heart_meanvar,
                                 vars.to.regress = c("nCount_RNA", "percent.mito"))
mouse_heart_meanvar <- RunPCA(mouse_heart_meanvar,
                              features = VariableFeatures(mouse_heart_meanvar))

# -------------------- Step 12: Elbow plot --------------------
p_elbow_meanvar <- ElbowPlot(mouse_heart_meanvar) + theme_classic()
...
mouse_heart_meanvar <- FindNeighbors(mouse_heart_meanvar, dims = 1:15)
mouse_heart_meanvar <- FindClusters(mouse_heart_meanvar, resolution = 1.0)

Idents(mouse_heart_meanvar) <- mouse_heart_meanvar$seurat_clusters
cluster_meanvar_ids <- Idents(mouse_heart_meanvar)
levels(cluster_meanvar_ids) <- as.character(as.numeric(levels(cluster_meanvar_ids)) + 1)
mouse_heart_meanvar$Cluster1based <- cluster_meanvar_ids
Idents(mouse_heart_meanvar) <- mouse_heart_meanvar$Cluster1based

# -------------------- Step 14: UMAP --------------------
mouse_heart_meanvar <- RunUMAP(mouse_heart_meanvar, reduction = "pca", dims = 1:15)

# -------------------- Step 15: tSNE --------------------
mouse_heart_meanvar <- RunTSNE(mouse_heart_meanvar, reduction = "pca", dims = 1:15)

# -------------------- Step 16: Save UMAP/tSNE (cluster labels) --------------------
p_umap_meanvar <- DimPlot(mouse_heart_meanvar, reduction = "umap", label = TRUE) + theme_classic()
ggsave(filename = file.path(output_dir_meanvar, "UMAP_FirstClusters_meanvar.png"),
       plot = p_umap_meanvar, width = 8, height = 6, dpi = 300)
rm(p_umap_meanvar)

p_tsne_meanvar <- DimPlot(mouse_heart_meanvar, reduction = "tsne", label = TRUE) + theme_classic()
...
emb_pca <- Embeddings(mouse_heart_meanvar, reduction = "pca")[, 1:15]
cluster_labels <- as.factor(mouse_heart_meanvar$Cluster1based)
dists <- dist(emb_pca)
...
# ================================= SingleR ==================================
# -------------------- Step 18: SingleR annotation (MCA heart ref) --------------------
...
sce <- as.SingleCellExperiment(mouse_heart_meanvar)
ref_mca_heart <- readRDS(ref_path)
pred_mca <- SingleR(test = sce, ref = ref_mca_heart, labels = ref_mca_heart$label.main)
mouse_heart_meanvar$MCA_heart_label <- pred_mca$labels

# -------------------- Step 19: Save SingleR outputs & plots --------------------
pred_df <- as.data.frame(pred_mca)
write.csv(pred_df, file.path(output_dir_singler, "SingleR_MCA_heart_Result.csv"), row.names = TRUE)

Idents(mouse_heart_meanvar) <- mouse_heart_meanvar$MCA_heart_label
p_umap_mca <- DimPlot(mouse_heart_meanvar, reduction = "umap", label = FALSE, repel = TRUE) + ggtitle("SingleR UMAP – MCA heart") + theme_classic()
p_tsne_mca <- DimPlot(mouse_heart_meanvar, reduction = "tsne", label = FALSE, repel = TRUE) + ggtitle("SingleR tSNE – MCA heart") + theme_classic()
ggsave(file.path(output_dir_singler, "UMAP_SingleR_MCA_heart.png"), p_umap_mca, width = 8, height = 6, dpi = 300)
ggsave(file.path(output_dir_singler, "tSNE_SingleR_MCA_heart.png"), p_tsne_mca, width = 8, height = 6, dpi = 300)
rm(p_umap_mca, p_tsne_mca)

saveRDS(mouse_heart_meanvar, file = file.path(output_dir_singler, "mouse_heart_annotated_MCA_heart.rds"))
utils::write.csv(as.data.frame(table(mouse_heart_meanvar$MCA_heart_label)),
                 file = file.path(output_dir_singler, "Counts_by_SingleR_Label.csv"), row.names = FALSE)

# -------------------- Step 20: DEGs per SingleR label --------------------
Idents(mouse_heart_meanvar) <- mouse_heart_meanvar$MCA_heart_label
deg_results <- FindAllMarkers(object = mouse_heart_meanvar, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
...
# -------------------- Step 21: Violin (Myeloid cell) --------------------
vln_dir1 <- file.path(output_dir_singler, "Violin_Markers_MyeloidCheck")
dir.create(vln_dir1, showWarnings = FALSE, recursive = TRUE)

subset_myeloid <- subset(mouse_heart_meanvar, subset = MCA_heart_label == "Myeloid cell")
subset_myeloid$MCA_heart_label <- droplevels(factor(subset_myeloid$MCA_heart_label))
...
plot_list <- lapply(marker_genes1, function(gene) {
  VlnPlot(subset_myeloid, features = gene, group.by = "MCA_heart_label", pt.size = 0) +
...
ggsave(filename = file.path(vln_dir1, "Violin_AllMarkers_Myeloid_Patched.png"), plot = p_combined, width = 30, height = 20, dpi = 600)
rm(p_combined); gc()
...
four_cell_for_violin <- subset(mouse_heart_meanvar, idents = c("Macrophage_C1qa high","Macrophage_Vsig4 high","Proliferating Macrophage","Monocyte"))
four_cell_for_violin$MCA_heart_label <- droplevels(factor(four_cell_for_violin$MCA_heart_label))
Idents(four_cell_for_violin) <- four_cell_for_violin$MCA_heart_label

marker_genes2 <- marker_genes1
for (gene in marker_genes2) {
  p <- VlnPlot(four_cell_for_violin, features = gene, group.by = "MCA_heart_label", pt.size = 0) +
    ggtitle(gene) + theme_classic(base_size = 12) +
...
Idents(mouse_heart_meanvar) <- mouse_heart_meanvar$MCA_heart_label
macro_subset <- subset(mouse_heart_meanvar, idents = c("Macrophage_C1qa high","Macrophage_Vsig4 high","Proliferating Macrophage"))
saveRDS(macro_subset, file = file.path(output_dir_recluster, "MouseHeart_MCA_Macrophages.rds"))

macro_subset <- FindVariableFeatures(macro_subset, selection.method = "mean.var.plot", mean.cutoff = c(0.02, 4), dispersion.cutoff = c(0.3, Inf))
...
macro_subset <- FindClusters(macro_subset, resolution = 0.15)
...
genes_to_plot <- c("Csf1r","Cd68","Adgre1","Fcgr1","Mrc1","Timd4","Folr2","Lyve1","Ccr2","Plac8","S100a8","H2-Aa","H2-Ab1","Top2a","Nusap1","Ccnb1","Cd3d")
...
ggsave(filename = file.path(output_dir_recluster, "Heatmap_Top30_MF_Subtype_50cells.png"), plot = p_heat, width = 13, height = 14, dpi = 600)

# -------------------- Step 27: Assign MF subtype final labels (manual) --------------------
mf_labels <- c("1" = "TLF⁺ Macrophage", "2" = "TLF⁺ Macrophage", "3" = "Proliferative Macrophage")
...
key_markers <- c("Timd4","Lyve1","Folr2","Ccr2","Plac8","H2-Aa","H2-Ab1","Top2a","Nusap1","Ccnb1")
p_feat_umap <- FeaturePlot(macro_subset, features = key_markers, reduction = "umap", order = TRUE, cols = c("lightgrey","red")) & theme_classic()
ggsave(filename = file.path(output_dir_recluster, "FeaturePlot_MFSubtypeMarkers_UMAP.png"), plot = p_feat_umap, width = 20, height = 16, dpi = 300)
p_feat_tsne <- FeaturePlot(macro_subset, features = key_markers, reduction = "tsne", order = TRUE, cols = c("lightgrey","red")) & theme_classic()
ggsave(filename = file.path(output_dir_recluster, "FeaturePlot_MFSubtypeMarkers_tSNE.png"), plot = p_feat_tsne, width = 20, height = 16, dpi = 300)
...
saveRDS(macro_subset, file = file.path(output_dir_recluster, "MouseHeart_MCA_MFSubtype_Final.rds"))
# ============================ END ================================
