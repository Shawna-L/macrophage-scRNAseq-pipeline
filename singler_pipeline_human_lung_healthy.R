# ================================================================
# SingleR-based macrophage annotation pipeline (human lung – healthy)
# ================================================================
# Author: Jingshuo Li
# Version: 1.0
# Data source: GEO GSE132771 (healthy human lung; samples NML1, NML2, NML3)
# Description: End-to-end scRNA-seq workflow for HUMAN LUNG (HEALTHY)
...
# data layout for GitHub repo
base_data_dir <- file.path("data", "human", organ, condition)
sample_ids    <- c("NML1", "NML2", "NML3")
...
ref_path      <- file.path("refs", "human", "lung_ref.rds")        # e.g., HCA lung reference as RDS
...
lung_healthy <- merge(
  ...
  project = "Lung_Healthy"
)

# -------------------- Step 4: Join layers (Seurat v5 compatibility) --------------------
lung_healthy <- JoinLayers(lung_healthy)

# -------------------- Step 5: Load human DAG gene list --------------------
dag_df      <- read.csv(dag_path, stringsAsFactors = FALSE)
dag_genes   <- unique(dag_df[[1]])
overlap_dag <- intersect(rownames(lung_healthy), dag_genes)

# -------------------- Step 6: Add QC metrics --------------------
# human mitochondrial genes: "^MT-"
lung_healthy[["percent.mito"]] <- PercentageFeatureSet(lung_healthy, pattern = "^MT-")
lung_healthy[["percent.dag"]]  <- PercentageFeatureSet(lung_healthy, features = overlap_dag)

print(summary(lung_healthy$percent.mito))
print(summary(lung_healthy$percent.dag))
print(summary(lung_healthy$nFeature_RNA))
...
hist(lung_healthy$percent.mito, breaks = 50, col = "lightblue", main = "Mito %", xlab = "percent.mito")
dev.off()

png(file.path(hist_dir, "Histogram_percent.dag.png"), width = 800, height = 600)
hist(lung_healthy$percent.dag, breaks = 50, col = "pink", main = "DAG %", xlab = "percent.dag")
dev.off()
...
p1 <- FeatureScatter(lung_healthy, feature1 = "nCount_RNA", feature2 = "percent.mito")
p2 <- FeatureScatter(lung_healthy, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
p3 <- FeatureScatter(lung_healthy, feature1 = "nCount_RNA", feature2 = "percent.dag")
p4 <- FeatureScatter(lung_healthy, feature1 = "percent.dag", feature2 = "percent.mito")
...
message("Raw cells: ", ncol(lung_healthy))
message("After QC filtering: ", ncol(lung_healthy_filtered))
message("Retention ratio: ", round(ncol(lung_healthy_filtered) / ncol(lung_healthy), 3))

# scDblFinder
sce <- as.SingleCellExperiment(lung_healthy_filtered)
sce <- scDblFinder(sce, samples = colData(sce)$sample)
lung_healthy_filtered$doublet_class <- colData(sce)$scDblFinder.class
lung_healthy_filtered$doublet_score <- colData(sce)$scDblFinder.score

dbl_dir <- file.path(results_dir, "Doublets")
dir.create(dbl_dir, showWarnings = FALSE, recursive = TRUE)
dbl_tab <- as.data.frame(table(lung_healthy_filtered$doublet_class))
...
lung_healthy_clean <- subset(lung_healthy_filtered, subset = doublet_class == "singlet")
saveRDS(lung_healthy_clean, file = file.path(results_dir, "lung_healthy_filtered_singlets.rds"))

# -------------------- Step 10: Normalize data & JoinLayers --------------------
lung_healthy_clean <- NormalizeData(lung_healthy_clean)
lung_healthy_clean <- JoinLayers(lung_healthy_clean)

# -------------------- Step 11: FindVariableFeatures (mean.var.plot) --------------------
lung_healthy_meanvar <- FindVariableFeatures(
  lung_healthy_clean,
...
message("Variable features: ", length(VariableFeatures(lung_healthy_meanvar)))

output_dir_meanvar <- file.path(results_dir, "Method_meanvar")
dir.create(output_dir_meanvar, recursive = TRUE, showWarnings = FALSE)

# -------------------- Step 12: Scale + PCA --------------------
lung_healthy_meanvar <- ScaleData(
  lung_healthy_meanvar,
  vars.to.regress = c("nCount_RNA", "percent.mito")
)
lung_healthy_meanvar <- RunPCA(
  lung_healthy_meanvar,
  features = VariableFeatures(lung_healthy_meanvar)
)

# -------------------- Step 13: Elbow plot --------------------
p_elbow <- ElbowPlot(lung_healthy_meanvar) + theme_classic()
...
lung_healthy_meanvar <- FindNeighbors(lung_healthy_meanvar, dims = 1:15)
lung_healthy_meanvar <- FindClusters(lung_healthy_meanvar, resolution = 1.0)

Idents(lung_healthy_meanvar) <- lung_healthy_meanvar$seurat_clusters
lvl <- Idents(lung_healthy_meanvar)
levels(lvl) <- as.character(as.numeric(levels(lvl)) + 1)
lung_healthy_meanvar$Cluster1based <- lvl
Idents(lung_healthy_meanvar) <- lung_healthy_meanvar$Cluster1based

# -------------------- Step 15: UMAP --------------------
lung_healthy_meanvar <- RunUMAP(lung_healthy_meanvar, reduction = "pca", dims = 1:15)

# -------------------- Step 16: tSNE --------------------
lung_healthy_meanvar <- RunTSNE(lung_healthy_meanvar, reduction = "pca", dims = 1:15)

# -------------------- Step 17: Save UMAP & tSNE (cluster labels) --------------------
p_umap <- DimPlot(lung_healthy_meanvar, reduction = "umap", label = TRUE) + theme_classic()
ggsave(file.path(output_dir_meanvar, "UMAP_FirstClusters_meanvar.png"),
       p_umap, width = 8, height = 6, dpi = 300)
rm(p_umap)

p_tsne <- DimPlot(lung_healthy_meanvar, reduction = "tsne", label = TRUE) + theme_classic()
...
emb_pca <- Embeddings(lung_healthy_meanvar, reduction = "pca")[, 1:15]
cluster_labels <- as.factor(lung_healthy_meanvar$Cluster1based)
dists <- dist(emb_pca)
...
sce_singler <- as.SingleCellExperiment(lung_healthy_meanvar)
ref_lung    <- readRDS(ref_path)

pred_lung <- SingleR(test = sce_singler, ref = ref_lung, labels = ref_lung$label.main)
lung_healthy_meanvar$HCA_lung_label <- pred_lung$labels
...
Idents(lung_healthy_meanvar) <- lung_healthy_meanvar$HCA_lung_label
p_umap2 <- DimPlot(lung_healthy_meanvar, reduction = "umap", label = FALSE, repel = TRUE) +
  ggtitle("SingleR UMAP – HCA Human lung") + theme_classic() + coord_fixed()
p_tsne2 <- DimPlot(lung_healthy_meanvar, reduction = "tsne", label = FALSE, repel = TRUE) +
...
saveRDS(lung_healthy_meanvar, file = file.path(output_dir_singler, "lung_healthy_annotated_HCA_lung.rds"))
write.csv(as.data.frame(table(lung_healthy_meanvar$HCA_lung_label)),
...
Idents(lung_healthy_meanvar) <- lung_healthy_meanvar$HCA_lung_label
macro_cells <- colnames(lung_healthy_meanvar)[grepl("macroph|kupffer|alveolar", lung_healthy_meanvar$HCA_lung_label, ignore.case = TRUE)]
lung_healthy_macro_subset <- subset(lung_healthy_meanvar, cells = macro_cells)
saveRDS(lung_healthy_macro_subset, file = file.path(output_dir_recluster, "HumanLung_HCA_Macrophages.rds"))

lung_healthy_macro_subset <- FindVariableFeatures(
  lung_healthy_macro_subset,
  selection.method = "mean.var.plot",
  mean.cutoff = c(0.1, 5),
  dispersion.cutoff = c(0.5, Inf),
  nfeatures = 5000
)
p_elbow_macro <- ElbowPlot(lung_healthy_macro_subset) + theme_classic()
ggsave(file.path(output_dir_recluster, "ElbowPlot_macro_subset.png"),
       p_elbow_macro, width = 6, height = 5, dpi = 300)
rm(p_elbow_macro); gc()

lung_healthy_macro_subset <- ScaleData(lung_healthy_macro_subset)
lung_healthy_macro_subset <- RunPCA(lung_healthy_macro_subset, features = VariableFeatures(lung_healthy_macro_subset))
lung_healthy_macro_subset <- FindNeighbors(lung_healthy_macro_subset, dims = 1:15)
lung_healthy_macro_subset <- FindClusters(lung_healthy_macro_subset, resolution = 0.2)

Idents(lung_healthy_macro_subset) <- lung_healthy_macro_subset$seurat_clusters
levels(Idents(lung_healthy_macro_subset)) <- as.character(as.numeric(levels(Idents(lung_healthy_macro_subset))) + 1)
lung_healthy_macro_subset$MF_subtype <- Idents(lung_healthy_macro_subset)
Idents(lung_healthy_macro_subset) <- lung_healthy_macro_subset$MF_subtype

lung_healthy_macro_subset <- RunUMAP(lung_healthy_macro_subset, dims = 1:15)
lung_healthy_macro_subset <- RunTSNE(lung_healthy_macro_subset, dims = 1:15)

ggsave(file.path(output_dir_recluster, "UMAP_MF_Subtype_Annotated.png"),
       DimPlot(lung_healthy_macro_subset, reduction = "umap", group.by = "MF_subtype", label = TRUE, repel = TRUE),
       width = 8, height = 6, dpi = 300)
ggsave(file.path(output_dir_recluster, "tSNE_MF_Subtype_Annotated.png"),
       DimPlot(lung_healthy_macro_subset, reduction = "tsne", group.by = "MF_subtype", label = TRUE, repel = TRUE),
       width = 8, height = 6, dpi = 300)

# -------------------- Step 23: MF subtype marker detection --------------------
mf_markers <- data.frame()
for (clust in levels(Idents(lung_healthy_macro_subset))) {
  cat("Finding MF markers for cluster", clust, "...\n")
  markers <- FindMarkers(lung_healthy_macro_subset,
                         ident.1 = clust,
                         only.pos = TRUE,
                         min.pct = 0.3,
                         logfc.threshold = 0.25)
...
genes_to_plot <- c(
  # general / TRM markers
  "CSF1R","CD68","ADGRE1","FCGR1A","MRC1",
  "FOLR2","LYVE1","TIMD4",
  # inflammatory / monocyte-related
  "S100A8","IL1B","CCR2","CXCL8","TNFAIP3","NFKBIZ",
  # LAM / remodeling
  "TREM2","FABP4","GPNMB","SPP1","MMP9","CD9",
  # proliferation
  "MKI67","TOP2A","NUSAP1",
  # MHC-II / IFN
  "HLA-DRA","HLA-DRB1","CD74","B2M","IFI6","STAT1"
)
for (gene in genes_to_plot) {
  p_v <- VlnPlot(lung_healthy_macro_subset, features = gene, group.by = "MF_subtype", pt.size = 0) +
...
for (stype in subtypes) {
  cells <- WhichCells(lung_healthy_macro_subset, idents = stype)
  if (length(cells) > 0) {
    cells_use <- c(cells_use, sample(cells, size = min(50, length(cells))))
  }
}

subset_obj <- subset(lung_healthy_macro_subset, cells = cells_use)
...
# EDIT: map current cluster numbers to your final labels
mf_labels <- c(
  "1" = "Inflammatory Macrophage",
  "2" = "ECM Remodeling & Lipid Handling Macrophage",
  "3" = "TLF-A Macrophage (Homeostatic)"
)

lung_healthy_macro_subset$MF_subtype_label <- plyr::mapvalues(
  x = Idents(lung_healthy_macro_subset),
  from = names(mf_labels),
  to   = mf_labels
)
Idents(lung_healthy_macro_subset) <- lung_healthy_macro_subset$MF_subtype_label

ggsave(file.path(output_dir_recluster, "UMAP_MF_Subtype_ManualLabel.png"),
       DimPlot(lung_healthy_macro_subset, reduction = "umap",
...
ggsave(file.path(output_dir_recluster, "tSNE_MF_Subtype_ManualLabel.png"),
       DimPlot(lung_healthy_macro_subset, reduction = "tsne",
               group.by = "MF_subtype_label", cols = mf_colors, label = FALSE, repel = TRUE) +
...
p_feat_umap <- FeaturePlot(lung_healthy_macro_subset, features = key_markers,
...
p_feat_tsne <- FeaturePlot(lung_healthy_macro_subset, features = key_markers,
...
deg_mf_final <- FindAllMarkers(
  object = lung_healthy_macro_subset,
  only.pos = TRUE,
  min.pct = 0.3,
  logfc.threshold = 0.25
)
...
subset_obj2 <- subset(lung_healthy_macro_subset, cells = cells_use2)
...
saveRDS(lung_healthy_macro_subset,
...
mf_counts <- as.data.frame(table(lung_healthy_macro_subset$MF_subtype_label))
...
p_bar <- ggplot(mf_counts, aes(x = MF_Subtype, y = Cell_Count, fill = MF_Subtype)) +
  ...
  ggtitle("Cell Number and Percentage per Macrophage Subtype (Lung Healthy)") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
